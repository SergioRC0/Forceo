...
export default function RegisterForm() {
    ...
    const onSubmit = async (registerDataForm) => {
        const {ok, data} = await registerUser(registerDataForm)
        if(ok){
            router.push('/login');
        }else{
  ...
  return (
    <div>
      <div className="flex-grow flex items-center justify-center md:mb-30">
          <form onSubmit={handleSubmit(onSubmit)} className="w-full max-w-md p-6 rounded-xl shadow-xl space-y-4 mx-4 bg-white">
            ...
            <div className="relative w-full">
              ...
              />
            {errors.email && ( <p className="absolute right-2 top-1/2 transform -translate-y-1/2 text-red-500 text-sm"> {errors.email.message} </p>)}
            </div>
            ...
            <div className="relative w-full">
              <input
                ...
                className="w-full border rounded-xl px-3 py-2 border-black text-black"
              />
            {errors.username && (<p className="absolute right-2 top-1/2 transform -translate-y-1/2 text-red-500 text-sm"> {errors.username.message} </p> )}
            </div>
            {/* Campo Password */}
            <div className="relative w-full">
              <input
                ...
                className="w-full border rounded-xl px-3 py-2 border-black text-black"
              />
              {errors.password && ( <p className="absolute right-2 top-1/2 transform -translate-y-1/2 text-red-500 text-sm"> {errors.password.message} </p> )}
            </div>
